...
public class PredatorSearchFood : MonoBehaviour {
	...
	private double[,] formatData(GameObject actualNode, GameObject[] neighbors){
		...
		for (int i = 0; i < neighbors.Length; i++) {
			nodesData[0,i] = neighbors[i].GetComponent<PathNode>().getPrays();
			nodesData[1,i] = actualNode.GetComponent<PathNode>().getPredators();
			nodesData[2,i] = 1;
		}
		//Agrega el nodo actual para ser procesado tambien
		nodesData[0, neighbors.Length ] = actualNode.GetComponent<PathNode>().getPrays();
